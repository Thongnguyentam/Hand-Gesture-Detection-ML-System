...
def get_args():
    ...
    parser.add_argument("--ws_url", help='WebSocket URL', type=str, default='ws://localhost:8000/asl/predict')
...
async def main():
    ...
    # A queue that stores the past 16 positions of the index fingertip (x, y), 
    # converted to relative coordinates and normalized.
    ...
    while True:
        ...
        if not ret:
            break
        image = cv.flip(image, 1)  # Mirror display, 1 means horizontal flip
        debug_image = copy.deepcopy(image) # Create a copy of the image for debugging

        # Detection implementation
        image = cv.cvtColor(image, cv.COLOR_BGR2RGB) # Convert the image from BGR to RGB

        image.flags.writeable = False # Set the image as not writeable to avoid overwriting it during processing
        results = hands.process(image) # Process the image with the hands model
        image.flags.writeable = True # Set the image as writeable again

        if results.multi_hand_landmarks is not None: # If the hands are detected
            for hand_landmarks, handedness in zip(results.multi_hand_landmarks,
                                                  results.multi_handedness): # For each hand
                
                # Bounding box calculation, This is where the hand is located in the image.
                brect = calc_bounding_rect(debug_image, hand_landmarks) # Calculate the bounding box of the hand
                
                # Landmark calculation
                landmark_list = calc_landmark_list(debug_image, hand_landmarks) # Calculate the landmark of the hand
                ...
                #print("finger_gesture_id: ", finger_gesture_id) 
                ...
                ############ Drawing part ############
                # Drawing part
                debug_image = draw_bounding_rect(use_brect, debug_image, brect) # use_brect: whether to draw the bounding box
                debug_image = draw_landmarks(debug_image, landmark_list)
                debug_image = draw_info_text(
                    debug_image,
                    brect,
                    handedness, # handedness: the handedness of the hand (left or right)
                    keypoint_classifier_labels[hand_sign_id], # the hand sign label
                    point_history_classifier_labels[most_common_fg_id[0][0]], # the finger gesture label
                )
        else:
            point_history.append([0, 0])
...
def select_mode(key, mode):
    number = -1
    if 48 <= key <= 57:  # 0 ~ 9 Detects Number Keys
        number = key - 48
    if key == 110:  # n
        mode = 0 # Normal mode
    if key == 107:  # k
        mode = 1 # Key point logging mode
    if key == 104:  # h
        mode = 2 # Point history logging mode
    return number, mode
...
def pre_process_point_history(image, point_history):
    ...
    for index, point in enumerate(temp_point_history):
        ...
        temp_point_history[index][0] = (temp_point_history[index][0] - base_x) / image_width # Normalize the x coordinate
        temp_point_history[index][1] = (temp_point_history[index][1] - base_y) / image_height # Normalize the y coordinate
...
def logging_csv(number, mode, landmark_list, point_history_list):
    ...
    if mode == 1 and (0 <= number <= 9):
        ...
        with open(csv_path, 'a', newline="") as f:
            writer = csv.writer(f)
            writer.writerow([number, *landmark_list]) # Write the landmark list to the csv file
