...
## Why We Need Distributed Tracing {#why-tracing}
...
## Our Jaeger Architecture {#architecture}
...
- **Port**: 16686 (Web UI)
- **Access**: `jaeger.34.63.222.25.nip.io` (via Ingress)
- **Features**:
...
### Our Direct Connection Approach
...
## Our FastAPI Implementation {#implementation}
...
### Step 1: Prepare Elasticsearch Access

Copy Elasticsearch credentials to our tracing namespace:
...
kubectl exec -n tracing deployment/jaeger-collector -- \
  curl -X GET "http://elasticsearch-master.logging.svc.cluster.local:9200/_cluster/health?pretty"
```

### Step 4: Test Application Tracing

```bash
# Generate test traces
for i in {1..5}; do 
  curl -s http://asl.34.63.222.25.nip.io/api/health > /dev/null && \
  echo "Request $i sent"
  sleep 1
done
...
### Accessing Our Jaeger UI

**Production**: http://jaeger.34.63.222.25.nip.io
**Development**: `kubectl port-forward -n tracing svc/jaeger-query 16686:16686`
...
kubectl port-forward -n tracing deployment/jaeger-collector 14269:14269
...
**Debug steps:**
```bash
# Test connectivity from Jaeger
kubectl exec -n tracing deployment/jaeger-collector -- \
  curl -X GET "http://elasticsearch-master.logging.svc.cluster.local:9200/_cluster/health"

# Check Elasticsearch credentials
kubectl get secret elasticsearch-master-credentials -n tracing -o yaml
...
curl -s http://localhost:14269/metrics | grep -E "(spans_received|spans_saved)"
